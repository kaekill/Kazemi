<?php
/**
 * WP_Auto_Manager_Forms class.
 */
class WP_Auto_Manager_Forms {
	...
	public function load_posted_form() {
		if ( ! empty( $_POST['auto_manager_form'] ) ) {
			$this->load_form_class( sanitize_title( $_POST['auto_manager_form'] ) );
		}
	}
	...
	private function load_form_class( $form_name ) {
		...
		if ( ! class_exists( 'WP_Auto_Manager_Form' ) )
			include( 'abstracts/abstract-wp-auto-manager-form.php' );

		// Now try to load the form_name
		$form_class  = 'WP_Auto_Manager_Form_' . str_replace( '-', '_', $form_name );
		$form_file   = AUTO_MANAGER_PLUGIN_DIR . '/includes/forms/class-wp-auto-manager-form-' . $form_name . '.php';
